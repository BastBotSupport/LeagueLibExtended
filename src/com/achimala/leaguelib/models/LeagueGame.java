...
public class LeagueGame implements PlayerList {
	...
	public LeagueGame(TypedObject obj, LeagueSummoner primarySummoner) {
		...
		for (Object o : obj.getArray("playerChampionSelections")) {
			TypedObject to = (TypedObject) o;
			if(to.getString("summonerInternalName").equals(primarySummoner.getInternalName())){
				List<Integer> spellList = new ArrayList<Integer>();
				spellList.add(to.getInt("spell1Id"));
				spellList.add(to.getInt("spell2Id"));
				_spellList = getSummonerSpellsFromIds(spellList);
			}
