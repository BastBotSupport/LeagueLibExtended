...
public class MainTest {
	...
	public static void main(String[] args) throws Exception {
		...
		c.getGameService().fillActiveGameData(sum);
		LeagueGame game = sum.getActiveGame();
		Map<String,Map<String,Object>> team = LeagueLibHelper.getSummonerTeamInfo(c, game, false);
...
/*
				...
				c.getPlayerStatsService().fillRankedStats(summoner, new Callback<LeagueSummoner>() {
					public void onCompletion(LeagueSummoner summoner) {
						...
						LeagueChampion champ = LeagueChampion.getChampionWithName("Soraka");
						Map<LeagueRankedStatType, Object> stats = summoner.getRankedStats().getAllStatsForChampion(champ);
						if (stats == null) {
							...
							System.out.println(summoner.getRankedStats().getKdrForChampion(champ));
							System.out.println();
						}
						System.out.flush();
						decrementCount();
				...
				c.getGameService().fillActiveGameData(summoner, new Callback<LeagueSummoner>() {
					...
					public void onError(Exception ex) {
						...
						if (ex instanceof LeagueException) {
							...
							System.out.println(((LeagueException) ex).getMessage());
						} else {
							System.out.println(ex.getMessage());
						}
						decrementCount();
